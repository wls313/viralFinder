...
def analyze_viral_traffic(trend_df: pd.DataFrame):
    ...
    latest_naver = trend_df['weight_naver'].iloc[-1] if 'weight_naver' in trend_df.columns else 0
    latest_google = trend_df['weight_google'].iloc[-1] if 'weight_google' in trend_df.columns else 0
